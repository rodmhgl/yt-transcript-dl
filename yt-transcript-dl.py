...
def format_transcript(
    transcript: List[Dict], format_type: str = "text", include_timestamps: bool = False
) -> str:
    """
    ...
    if format_type == "text":
        ...
        for i, entry in enumerate(transcript, 1):
            ...
            vtt_lines.extend(
                [f"{start} --> {end}", entry["text"], ""]  # Empty line between entries
            )
        return "\n".join(vtt_lines)
...
def get_default_extension(format_type: str) -> str:
    ...
    """
    extensions = {"text": ".txt", "json": ".json", "srt": ".srt", "vtt": ".vtt"}
...
def main():
    ...
    try:
        ...
        formatted_transcript = format_transcript(
            transcript, args.format, args.add_timestamps
        )
        ...
        if args.output_file:
            ...
            not args.clipboard
        ):  # Only print if not copying to clipboard (unless verbose)
